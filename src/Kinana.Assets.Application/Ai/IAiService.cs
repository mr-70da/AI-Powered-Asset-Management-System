...
public sealed class AiService : IAiService
{
    ...
    {
        // Answers are cached per role (R5.4): an Admin answer that could
        // contain cost figures is never served to a User and vice versa.
    ...
    private async Task<AssetSearchIntent> ExtractIntentAsync(string question, CancellationToken ct)
    {
        ...
        {
            // A provider outage must surface as a clear 503, never as a 500 or
            // a raw provider error (R4.5). The exception middleware turns it
            // into a friendly problem+json response.
            throw;
        }
        catch (Exception ex)
        {
            // The model returned something we could not interpret. Rather than
            // failing the request, fall back to a graceful out-of-scope answer.
    ...
    {
        // R4.3 / R2.6: cost figures are Admin-only. A User asking about value
        // gets a friendly refusal without any number, not a masked one.
        if (!includeCost)
        {
